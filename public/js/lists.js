function ListsViewModel() {
    ...
    self.mailContent = ko.computed(function() {
        if (self.listToShare())
             return "Dear whatsyourname\n\nSomeone wants to equal out your relationship by sharing this EquallyDo list with you. Show that someone that you are the one that takes care of things by subscribing to this list: \n" + window.location.hostname +"/api/connectToList/" + self.listToShare()._id + "\n\nNow go show that someone who is the real household hero!";
        else
            return "";
    }, this);
